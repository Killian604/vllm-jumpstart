"""
Shortcuts for poking vLLM. Functions that begin with underscore are not intended for general use.
"""
...
# Plumbing commands. Intended for simplification and not general use.

def _get_models_response(host, port) -> requests.Response:
    """

    Example Response.json() looks like:
    ...
    for i, chunk in enumerate(r.iter_lines(chunk_size=8192, decode_unicode=False, delimiter=b"data:",)):
        ...
        if chunk_decoded:
            ...
            if 'choices' not in data:
                raise ValueError(f'Not "choices" available: {data=} \n\n {json.dumps(data, indent=4)} // usually begets from user error')
...
def yield_streaming_response(chat: List[dict], model, api_url, stream: bool, max_tokens=DEFAULT_MAX_TOKENS, temperature: float = 1.0, ) -> Tuple[str, str]:
    """
    Stream an LLM response via vLLM. Ready to rock for Gradio use.
...
def generate_response(chat: List[dict], model, api_url, max_tokens=DEFAULT_MAX_TOKENS, temperature: float = 1.0, ) -> Tuple[str, str]:
    """
    Lazy method to fetch non-streaming result. I don't use this method much/at all.
    """
    try:
        content, role = next(yield_streaming_response(chat=chat, model=model,api_url=api_url, stream=False, max_tokens=max_tokens, temperature=temperature))
